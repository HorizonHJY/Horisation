...
| Medium | Chat history stops at 100 messages | `get_chat_history` returns the last 100 and the pane has no "load earlier". Fine for 11 people today; add a cursor (`before=<id>`) and a "Load earlier" row at the top of `.fr-msgs` when a thread grows past that. Also worth a "↓ new messages" pill when a message arrives while the reader is scrolled up (the pane stops auto-following at that point on purpose). |
