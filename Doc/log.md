...
服务端在洗牌那一刻定好——你挑的是"哪一张"，不是"什么牌"，和真实占卜一致。

在此之前，2026-09-08 新增 **塔罗牌 section v1**：`/tarot`，78 张 Rider–Waite–Smith 牌摊成扇形，
...
| 2026-09-11 | ~~桌面端导航从常驻 240px 侧栏改为 14px 书脊 + 悬停/`[` 滑出的 overlay 面板，可钉住~~ **（09-20 撤销，见下一行）** | 用户不想菜单常驻；三个方向里书脊保住了一步到位的导航、把宽度还给页面、且与文艺极简的身份一致 | 新用户第一次得发现左缘会响应（面包屑和书脊上的点在提示）；钉住/取消是整页布局跳变，刻意不做动画 |
...
| 2026-09-09 | 抽完自动把牌堆收拢并降到 34% 透明度 | 三张正面朝上的牌才是结果，75 张背面朝上的牌不该是页面上最亮的东西 | 多一段收拢动画；好处是下一次洗牌的起点已经摆好 |
...
- **Root Cause**: 每次 `pointermove` 都对**正在被 3D 变换的那个元素**调
  `getBoundingClientRect()`，用它算指针的相对位置。但元素一旦倾斜，它的包围盒就变了
  ——实测倾斜 9°/12° 时宽度缩 2.9px、左边移 1.4px、上边移 0.8px。于是：测量 → 倾斜 →
  盒子变了 → 同一个指针位置算出新的相对位置 → 新的倾斜 → …… 光标完全不动也会持续震荡。
  同理，把指针事件挂在会动的元素上，命中测试也会反馈（倾斜把元素挪开 → `pointerleave`
  → 回正 → 又命中 → 倾斜）
- **Reusable Solution**: 参考 `micku7zu/vanilla-tilt.js`（MIT）的做法——**测量对象必须是
  一个不动的参照**，而且**在 `pointerenter` 时量一次缓存下来**，不要每帧量。本项目做得更
  彻底：加一个 `.tarot-inspect__hit` 静态框，尺寸与卡片相同、永不变换，指针事件和测量都挂
  在它身上，会动的东西全部放在它里面。这样测量和命中测试都不可能反馈
- **Related**: vanilla-tilt 还确认了两件我们已经在做的事：跟手时不能有 transition（它在
  `mouseenter` 打开过渡、`setTimeout` 到时立刻清掉），以及 `pointermove` 要用 rAF 合并
...
### 2026-09-20 — 侧边栏：书脊撤了，改按钮

#### Goal
用户看了上线九天的书脊版之后："不太喜欢。路径不要显示；左边不要鼠标悬停自动展开收起，
设计成按钮的格式；或者其他现代一点的网站怎么做的？"

#### Direction
现代站点就两种做法，都是按钮、都没有悬停：Notion / Linear / Claude.ai 收起后留一条图标栏，
ChatGPT 收起后什么都不留、只在左上角留一个按钮。用户要的是"不常驻"，所以是后者。

#### Findings
1. **上一版的问题是我在替用户做他没要的决定。** 书脊上的分区点、红灯、面包屑，都是
   "收起了怎么知道自己在哪"这个我自己提出的问题的答案。用户的答案是：不需要知道，
   要看就点开。删掉之后 Layout 从 130 行回到 70 行。
2. **一个控件，两个位置。** ☰ 在顶栏左侧（收起时可见），« 在侧栏头部（展开时可见）——
   同一个 toggle，跟着面板走，这是 ChatGPT 的做法，比一个固定位置的按钮在页面上跳来跳去好。
3. **开合不做动画。** 上一版 detector 报过 `transition: margin-left`（整页布局逐帧动）。
   这次直接不动画：一个有意的点击得到一个干脆的结果。VS Code 也是这么做的。
4. **`inert` 保留。** 收起的侧栏仍然 `inert`，Tab 不会走进看不见的菜单。
5. **手机端一行没改。** 同一个 ☰ 在窄屏开抽屉、宽屏停靠侧栏，靠 `matchMedia` 分流。
6. **`nav.js` 里的 `locate()` 一并删了**——面包屑是它唯一的读者。

#### Verification
真实 App 挂 harness：桌面默认 `data-nav=closed`、侧栏 translateX(-100%) 且 inert、顶栏与
内容 0 偏移、无面包屑无书脊、只有 ☰；点 ☰ → 停靠 240、☰ 消失、« 出现、localStorage=open；
点 « → 收起、☰ 回来；`[` 往返；375px 下 ☰ 开抽屉带遮罩、点遮罩关闭、« 不显示。
构建干净，detector 四个文件均无发现。

---

### 2026-09-12 — 塔罗：先想问题再开始，边抽边翻，配上中文

#### Goal
用户的四条：① 按钮不叫 Shuffle the deck，叫 Start；② 按钮上方写清楚"闭上眼，心中想着
问题，默念三次，再点 Start"；③ 洗牌时就说 Shuffling / 洗牌中；④ 每点一张牌就直接翻开，
不要攒到最后；⑤ 牌名和释义要有中文。

#### Findings
1. **没有开源的中文塔罗数据。** 搜了 GitHub：`metabismuth/tarot-json`、`ekelen/tarot-api`、
   `Tarotoo-com/tarotoo-tarot-dataset` 全是英文；中文牌义只在版权站点上。于是中文层自己写，
   放在 `scripts/tarot_add_zh.mjs` 里可审可改：78 个牌名按固定惯例（大阿卡纳 22 个定名，
   小阿卡纳 = 花色 + 数字/侍从/骑士/王后/国王），关键词是通行的正位含义，每张一行。
   **明确不是 Waite 原文的翻译**——界面上中文关键词在前、Waite 英文在后，字体不同。
2. **边抽边翻只是把 `setRevealedCount` 从"第三张之后统一调度"挪到 `land()` 里。**
   落位 320ms 后翻面，第三张翻完才进 `done`。`revealing` 阶段整个删掉。
3. **释义随翻面逐条出现**（`slots.slice(0, revealedCount)`），不再等 `done`。
   `aria-live` 区域会逐条播报，正是想要的。

#### Verification
真组件挂 harness：初始文案与 Start 按钮正确；点 Start → "Shuffling… 洗牌中"；点第一张 →
立刻翻面、名下出现"Strength 力量"、释义区只有一条且带中文关键词、提示变 "2 to go."；
三张齐 → `done`、牌堆收拢、"Ask again"；检视弹层显示"Queen of Cups / 圣杯王后"及中文关键词。
78/78 都有 `name_zh` 且互不重复。构建、detector 均干净。
...
#### Follow-up 2（2026-09-10，第二轮）
上一轮改完，用户仍反馈"不连贯不丝滑，感觉在不断动态刷新，不断倾斜然后正位"。这是另一个
问题，而且是根子上的那个：**每帧对正在被变换的元素本身测量**，见 Pattern 17。
按用户要求去 GitHub 找了现成参考——`micku7zu/vanilla-tilt.js`（MIT），确认了两点：
测量对象要缓存、且不能是会动的那个元素。本项目据此加了 `.tarot-inspect__hit` 静态框，
指针事件与测量都挂在它上面。
实测：静态框在 0°/9°/14° 三种倾斜下包围盒完全不变（361.7×620 @ 105,140），
而原先测量的那个元素在同样条件下宽度漂 -2.9px、左边漂 +1.4px——这就是震荡的来源。
鼠标停住不动时 transform 连续三次采样完全一致。

#### Follow-up 1（2026-09-10，第一轮）
...
### 2026-09-09 — 塔罗牌：把"抽牌"变成一件你亲手做的事

#### Goal
提示先在心里想一个问题 → 点开始，牌堆洗牌展开 → 鼠标划过牌有回应 → 自己点三张。

#### Trigger / Context
v1 是"点一个按钮，三张牌自己出现"。牌是对的，但整件事没有仪式感——用户没有参与，
只是收到了结果。

#### Findings
1. **一条 78 张的弧线里，每张牌只露 10px，根本瞄不准。** 改成分行：宽屏 2 行 39 张，
   窄屏 3 行 26 张。行用 `transform` 定位（不是 `top`），所以牌堆、洗牌、展开三种状态
   共用同一块高度，下面的内容一次都不会跳。
2. **牌位全部走 transform，没有一处碰布局。** 每张牌的座位是 JS 按测得的行宽算成 px 后
   写进 `--seat-x/y/rot`，hover 抬升写 `--lift`，邻牌让位写 `--push-x`，最后合成一条
   transform。78 个节点全程在合成层上。
3. **"选中感"来自邻牌让位，不是被选中的那张牌自己变大。** 鼠标进入某张牌时，只给左右
   各三张写一次 `--push-x`（共六次属性写入，零 re-render），缝隙就打开了——这是让人相信
   "我正要拿这一张"的关键。
4. **落位必须是同一张牌在移动。** 点击时量出牌在扇形里的**中心**（不是
   `getBoundingClientRect` 的左上角——牌是斜的，包围盒比牌宽），再用 WAAPI 飞到牌位中心，
   中途抬高 46px。飞出去的牌用 `position: fixed`，免得被 `.tarot` 的 `overflow: hidden` 剪掉。
5. **`calc()` 里的自定义属性会静默失效。** 一开始把座位、让位、抬升全塞进
   `calc(var(--x) + var(--push-x))`；调试时误以为不生效，实际上是 transition 正在进行、
   `getComputedStyle` 读到的是起始值。教训：**带 transition 的元素，读 computed transform
   读到的是当前动画值，不是目标值**——验证要等动画结束，或直接读自定义属性本身。
6. **78 个 drop shadow 落在同一点会变成一团黑云。** 牌堆状态下把每张牌的阴影压到
   `0 1px 2px`，只给最上面那张（`:last-child`）留真正的投影。
7. **读牌出来以后，牌堆成了页面上最亮的东西。** 抽完自动把牌收拢回牌堆并压到 34% 透明度
   ——顺带把下一次洗牌的起点摆好了。
8. **拿掉一张牌后，键盘用户会掉到 `<body>`。** 焦点所在的牌被卸载，roving tabindex 指向
   一个不存在的座位。改成：后面那张牌滑进空出来的座位，焦点和 tab stop 都留在那个座位上。

#### Verification
真组件（不是复刻的 markup）挂在临时 harness 里跑通：1280 与 375 两种宽度下，
洗牌 → 三行/两行展开 → hover 抬升与邻牌让位（实测 `--push-x` 为 -7/-14/-21px）→
点选飞入 → 依次翻面 → 释义落地 → 牌堆收拢。窄屏"点空处"回退到最近的牌（实测点在
行内空白处仍成功取牌）。`prefers-reduced-motion` 全程走通：无洗牌动画、无飞行、
直接落位并翻开。构建干净，detector 两个文件均无发现。
